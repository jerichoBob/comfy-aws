...
export default function App() {
  ...
  useEffect(() => {
    if (workflows.length > 0 && !workflow) {
      const preferred = workflows.includes("txt2img-sdxl")
        ? "txt2img-sdxl"
        : workflows[0];
  ...
  useEffect(() => {
    if (models.checkpoints.length > 0 && !checkpoint)
      setCheckpoint(models.checkpoints[0]);
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100 overflow-hidden">
      ...
      <aside className="w-64 shrink-0 border-l border-zinc-800 overflow-y-auto bg-zinc-900">
        ...
        <JobHistory
          ...
          onRetry={(params) =>
            submit(String(params.workflow_id ?? workflow), params)
          }
