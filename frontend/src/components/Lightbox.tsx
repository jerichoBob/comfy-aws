        ...
        {meta && (
          <div className="w-64 shrink-0 bg-zinc-900/90 border border-zinc-700 rounded-lg p-4 text-sm text-zinc-300 flex flex-col gap-3 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-2">
              {meta.sampler_name && (
                <Row label="Sampler" value={String(meta.sampler_name)} />
              )}
              {meta.scheduler && (
                <Row label="Scheduler" value={String(meta.scheduler)} />
              )}
              {meta.steps != null && (
                <Row label="Steps" value={String(meta.steps)} />
              )}
              {meta.cfg != null && <Row label="CFG" value={String(meta.cfg)} />}
              {meta.seed != null && (
                <Row label="Seed" value={String(meta.seed)} />
              )}
