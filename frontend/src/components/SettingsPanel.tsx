...
function DimSelect({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (
    <div className="flex flex-col gap-1">
      <label className="text-xs font-medium text-zinc-400 uppercase tracking-wide">{label}</label>
      <select
...
export function SettingsPanel({ steps, cfg, seed, width, height, onStepsChange, onCfgChange, onSeedChange, onWidthChange, onHeightChange }: Props) {
  const randomizeSeed = () => onSeedChange(Math.floor(Math.random() * 2 ** 32))

  return (
    <div className="flex flex-col gap-5">
      <SliderRow label="Steps" value={steps} min={1} max={150} step={1} onChange={onStepsChange} />
      <SliderRow label="CFG" value={cfg} min={1} max={20} step={0.5} onChange={onCfgChange} />
